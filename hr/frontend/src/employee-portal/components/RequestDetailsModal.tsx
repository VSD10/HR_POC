      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] crystal-glass rounded-2xl shadow-2xl border border-white flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150 z-10">
        ...
        <div className="p-5 sm:p-6 border-b border-white/70 flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            ...
            <div className="text-[12px] text-[#64748B] flex flex-wrap items-center gap-2">
              <span className="flex items-center gap-1 font-medium text-teal-800 bg-teal-50/80 px-2 py-0.5 rounded-md border border-teal-200/50">
                <ShieldCheck className="w-3.5 h-3.5 text-teal-600" />
                <span>Assigned: {request.assignedTo || 'Triage Queue (HR Operations)'}</span>
              </span>
              <span>•</span>
              <span>Updated: {request.lastUpdated || 'Just now'}</span>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
          ...
          <div className="space-y-3">
            <h4 className="text-[12px] font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
              ...
              <span>Conversation & Comment Thread</span>
            </h4>

            {request.comments.length === 0 ? (
              <p className="text-[13px] text-slate-400 italic">
                No comments yet. You can post a response below.
              </p>
            ) : (
              <div className="space-y-3">
                {request.comments.map((c, idx) => (
                  <div
                    ...
                    className={`p-3.5 rounded-xl text-[13px] border ${
                      c.isHr
                        ? 'bg-teal-50/80 border-teal-200 text-[#0F172A] shadow-xs'
                    ...
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-[13px] flex items-center gap-1.5">
                          {c.author}
                          {c.authorId && (
                            <span className="text-[10px] text-slate-400 font-mono">({c.authorId})</span>
                          )}
                          {c.isHr ? (
                      ...
                      <span className="text-[11px] text-slate-400">{c.time || 'Just now'}</span>
                    </div>
                    <p className="text-[13px] leading-normal pl-7">{c.text}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="p-4 sm:p-5 border-t border-white/70 bg-white/40">
          <form onSubmit={handleSendReply} className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-[#0F172A] text-white hover:bg-slate-800 disabled:opacity-50 text-[13px] font-semibold flex items-center gap-1.5 transition-all shadow-sm cursor-pointer"
            >
              <span>Send</span>
