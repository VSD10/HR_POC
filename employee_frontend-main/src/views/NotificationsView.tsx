...
export const NotificationsView: React.FC<NotificationsViewProps> = ({
  ...
  return (
    <div className="w-full max-w-[1560px] mx-auto space-y-6">
      <div className="crystal-glass rounded-2xl p-6 shadow-glass flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-white/80 dark:border-white/10">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-[24px] font-bold text-[#0F172A] dark:text-white">Notifications</h1>
            {notifications.filter((n) => !n.read).length > 0 && (
              <span className="bg-[#0D9488] dark:bg-teal-600 text-white text-[11px] font-bold px-2 py-0.5 rounded-full">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-[13px] font-semibold border transition-all ${
              filterUnreadOnly
                ? 'bg-teal-50 dark:bg-teal-500/20 text-[#0D9488] dark:text-teal-400 border-teal-200 dark:border-teal-500/30'
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#0F172A] dark:bg-teal-600 hover:bg-slate-800 dark:hover:bg-teal-500 text-white text-[13px] font-semibold transition-all shadow-sm"
          >
      ...
      {/* Feed */}
      <div className="crystal-glass rounded-2xl shadow-glass p-4 sm:p-6 space-y-2.5 border border-white dark:border-white/10">
        {displayedNotifs.map((item) => (
          <div
            ...
            className={`p-4 rounded-xl transition-all flex items-start justify-between gap-4 cursor-pointer border ${
              !item.read
                ? 'bg-white/80 dark:bg-slate-800/80 hover:bg-white dark:hover:bg-slate-800 border-white dark:border-white/10 shadow-2xs'
                : 'hover:bg-white/60 dark:hover:bg-slate-800/50 border-transparent text-slate-700 dark:text-slate-300'
            }`}
          >
            <div className="flex items-start gap-3.5 min-w-0">
              <span
                className={`w-2.5 h-2.5 mt-1.5 rounded-full flex-shrink-0 ${
                  !item.read ? 'bg-[#0D9488] dark:bg-teal-400' : 'bg-slate-300 dark:bg-slate-600'
                }`}
              />
            ...
            <button className="text-slate-400 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 p-1 flex-shrink-0">
        ...
        {displayedNotifs.length === 0 && (
          <div className="p-12 text-center text-slate-500 dark:text-slate-400">
            ...
            <p className="text-[15px] font-medium text-[#0F172A] dark:text-white">All caught up!</p>
            <p className="text-[13px] text-slate-400 dark:text-slate-400">No unread notifications at this time.</p>
          </div>
        )}
      </div>
    </div>
