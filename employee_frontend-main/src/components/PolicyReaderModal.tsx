...
export const PolicyReaderModal: React.FC<PolicyReaderModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl crystal-glass rounded-2xl shadow-2xl border border-white dark:border-white/10 p-6 z-10 max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto py-5 space-y-4 pr-1">
          {policy.image && (
            <div className="w-full h-48 rounded-xl overflow-hidden shadow-xs border border-white dark:border-white/10">
              <img
                src={policy.image}
                alt={policy.title}
        ...
        {/* Footer */}
        <div className="pt-4 border-t border-white/70 dark:border-white/10 flex items-center justify-between">
          <span className="text-[12px] text-slate-500 dark:text-slate-400">
            Questions? Ask via <strong className="text-[#0D9488] dark:text-teal-400">Ask HR Assistant</strong>
          </span>
          <button
            onClick={onClose}
